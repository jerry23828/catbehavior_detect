    ...
    if request.method == 'POST':
        ...
        if 'Uheadshot' in request.files and request.files['Uheadshot'].filename != '':
            ...
            if file and allowed_file(file.filename):
                ...
                upload_path = os.path.join('uploadusers', filename)
                file.save(upload_path)
                Uheadshot = upload_path
            else:
...
def update_info():
    if request.method == 'POST':
        ...
        Uphone = data['Uphone']
        Uid = session.get('Uid')
        if 'Uheadshot' in request.files and request.files['Uheadshot'].filename != '' and allowed_file(request.files['Uheadshot'].filename):
            file = request.files['Uheadshot']
            filename = secure_filename(file.filename)
            upload_path = os.path.join('uploadusers', filename)
            file.save(upload_path)
...
def dashboard():
    ...
    return render_template('behavedetect.html', user=user_info)
